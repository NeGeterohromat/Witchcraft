...
namespace PixelRPG
{
    public class UserView : Form
    {
        ...
        public void OpenInformationLabel()
        {
            var info = new Label()
            {
                AutoSize = true,
                Text = 
@"1)Управление(Играть на английской раскладке):
а) W,A,S,D - Вперёд, влево, назад, вправо
б) Инвентарь
I - Открыть инвентарь
J - Поменять местами выделенные элементы инвентаря
P - Съесть еду
C - Создать новый предмет в окне создания из выложенных предметов
E - При закрытом инвентаре взять в слот руки первый попавшийся предмет
в) Взаимодействие с миром
P - поставить предмет из слота руки/открыть инвентарь предмета спереди
L - Сломать предмет спереди/ударить сущность спереди
M - Использовать магию из текущего слота магии
г) 1,2,3,4,5,6,7,8,9 - выбрать n-ый слот магии
д) Esc - Выйти из игры в меню паузы
2) Крафты - создание новых предметов из уже собранных.
Тукцщие крафты (Попробуйте найти расположение предметов в окне крафтов самостоятельно):
2 Сердца зомби -> Магическая пыль
3 Магической пыли + Дерево -> Стол Магии
Палка + Камень -> Рубило
4 Дёрна -> Блок земли
Дерево + Рубило -> Блок Досок
Блок Досок + Рубило -> Сундук
3) Создание магии:
В столе магии в строке заклинаний можно писать свои заклинания и получать их
Догадайтесь сами, по какой логике они устроены. Вот несколько примеров:
lfvf;m gznm jlby jlby ldf jlby jlby ldf
даст заклинание урона со следующей зоной урона:
20002
00000
00000
00000
20002
А заклинание yfqlb WoodenChest
Покажет путь от игрока до ближайшего деревянного сундука"
            };
            var table = new TableLayoutPanel() {BackColor=Color.Wheat,Dock=DockStyle.Fill};
            var exitbutton = Menu.CreateMenuButton("Выход", (sender, e) => Controls.Remove(table));
            ...
            table.Controls.Add(exitbutton, 0, 1);
            Controls.Add(table);
            table.BringToFront();
        }
        ...
        public void OpenGame(GameModel game)
        {
            ...
            keyBar.KeyPress += (sender, e) => controls.CheckKeyCommands(e.KeyChar);
            ...
            worldTimer.Interval = GameModel.WorldTick;
            worldTimer.Tick += (sender, e) => controls.GetWorldTimer();
            worldTimer.Start();
            GC.Collect();
        }
        ...
        {
            visual.StopTimer += () => worldTimer.Stop();
        ...
        public void OpenMenu(MenuType type)
        {
            ...
            switch (type)
            {
                ...
                case MenuType.Escape:
                    ...
                    break;
                case MenuType.Save:
                    Controls.Add(saveMenu.MenuTable);
                    break;
            }
        }
    }
